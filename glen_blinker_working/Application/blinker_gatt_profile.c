...
/*
   ...
   if the data has changed.
      blinker_ReadAttrCB  - callback function for reading an attribute
      blinker_WriteAttrCB - callback fucntion for writing an attribute
...
/* local include files */
#include  "blinker_gatt_profile.h"




/*
   blinker_ReadAttrCB(uint16_t, gattAttribute_t *, uint8_t *, uint16_t *,
                      uint16_t, uint16_t, uint8_t)

   Description:      This function is the callback function for reading an
                     attribute for the blinker profile.  It just gets the
    ...
    if (offset == 0)  {
        ...
        if (pAttr->type.len == ATT_BT_UUID_SIZE)  {
            ...
            switch (uuid)  {
                ...
                case  BLINKERPROFILE_REDLED_UUID:
                case  BLINKERPROFILE_GREENLED_UUID:
                ...
                case  BLINKERPROFILE_REDTOGGLE_UUID:
                case  BLINKERPROFILE_GREENTOGGLE_UUID:
                        /* can't read red and green LED toggles */
                        *pLen = 0;
                        break;
...
/*
   blinker_WriteAttrCB(uint16_t, gattAttribute_t *, uint8_t *, uint16_t,
                       uint16_t, uint8_t)

   Description:      This function is the callback function for writing an
                     attribute for the blinker profile.  It writes the passed
                               ...
                               uint8_t method)
{
                    ...
                    if (offset == 0)  {
                        ...
                        if (len == 1)  {
                            ...
                            *((uint8 *)(pAttr->pValue)) = pValue[0];
                            ...
                            if(uuid == BLINKERPROFILE_REDLED_UUID)
                                /* changed the red LED value */
                                changeID = BLINKERPROFILE_REDLED;
                            else
                                /* changed the green LED value */
                                changeID = BLINKERPROFILE_GREENLED;
                        }
                        else  {
    ...
    if ((changeID != NO_CHANGE) && (BlinkerProfile_AppCBs != NULL) &&
        (BlinkerProfile_AppCBs->pfnSimpleProfileChange != NULL))

        /* have a callback and there was a change, let the peripheral know */
        BlinkerProfile_AppCBs->pfnSimpleProfileChange(changeID);
