...
static void BarebotPeripheral_init(void)
{
    ...
    HCI_LE_WriteSuggestedDefaultDataLenCmd(BS_SUGGESTED_PDU_SIZE,
                                           BS_SUGGESTED_TX_TIME);

    /* initialize GAP layer for Peripheral role and register to receive GAP events */
    GAP_DeviceInit(GAP_PROFILE_PERIPHERAL, selfEntity, DEFAULT_ADDRESS_MODE,
                   &pRandomAddress);
...
static void BarebotPeripheral_taskFxn(UArg a0, UArg a1)
{
    ...
    for (;;)
    {
        ...
        events = Event_pend(syncEvent, Event_Id_NONE, BS_ALL_EVENTS,
                                      ...
                                      (void**) &pMsg) == ICALL_ERRNO_SUCCESS)
            {
                ...
                if ((src == ICALL_SERVICE_CLASS_BLE) && (dest == selfEntity))
                {
                    ...
                    if (((ICall_Stack_Event*) pMsg)->signature != 0xffff)
                        ...
                        safeToDealloc = BarebotPeripheral_processStackMsg(
                                (ICall_Hdr*) pMsg);
                }
...
static uint8_t BarebotPeripheral_processStackMsg(ICall_Hdr *pMsg)
{
    ...
    case GATT_MSG_EVENT:
        ...
        GATT_bm_free(&((gattMsgEvent_t*) pMsg)->msg,
                     ((gattMsgEvent_t*) pMsg)->method);
        break;
...
static void BarebotPeripheral_processGapMessage(gapEventHdr_t *pMsg)
{
    ...
    case GAP_DEVICE_INIT_DONE_EVENT:
        ...
        if (((gapDeviceInitDoneEvent_t*) pMsg)->hdr.status == SUCCESS)
        {
            ...
            BS_VERIFY(
                    GapAdv_create(&BarebotPeripheral_advCallback, &advParams1, &advHandleLegacy));
            /* load advertising data for set #1 (setup in sysconfig) */
            BS_VERIFY(
                    GapAdv_loadByHandle(advHandleLegacy, GAP_ADV_DATA_TYPE_ADV, sizeof(advData1), advData1));
            ...
            BS_VERIFY(
                    GapAdv_create(&BarebotPeripheral_advCallback, &advParams2, &advHandleLongRange));
            /* load advertising data for set #2 (setup in sysconfig) */
            BS_VERIFY(
                    GapAdv_loadByHandle(advHandleLongRange, GAP_ADV_DATA_TYPE_ADV, sizeof(advData2), advData2));
            /* set event mask for set #2 */
            BS_VERIFY(
                    GapAdv_setEventMask(advHandleLongRange, GAP_ADV_EVT_MASK_START_AFTER_ENABLE | GAP_ADV_EVT_MASK_END_AFTER_DISABLE | GAP_ADV_EVT_MASK_SET_TERMINATED));
...
void ButtonPressed(uint8_t buttonId)
{
    bpEvtData_t data;
    data.byte = buttonId;
    BarebotPeripheral_enqueueMsg(BS_BUTTON_PRESSED, data);
}
...
static void BarebotPeripheral_advCallback(uint32_t event, void *pBuf,
                                          uintptr_t arg)
{
    /* variables */
...
static bool BarebotPeripheral_processAdvEvent(bpEvtData_t eventData)
{
    ...
    if (((bpGapAdvEventData_t*) (eventData.pData))->event
